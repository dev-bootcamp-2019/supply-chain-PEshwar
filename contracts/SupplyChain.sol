...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier isForSale(State _state) {
      require(_state == State.ForSale);
      _;
  }
  modifier isSold(State _state) {
      require(_state==State.Sold);
    _;
      
  }
  modifier hasShipped(State _state) {
      require(_state == State.Shipped);
      _;
  }
  modifier isReceived(State _state) {
  ...
  function buyItem(uint sku)
    public payable isForSale(items[sku].state) paidEnough(items[sku].price) checkValue(sku)
  {
  ...
  function shipItem(uint sku)
    public isSold(items[sku].state) isSeller(sku) 
  {
  ...
  function receiveItem(uint sku)
    public hasShipped(items[sku].state) isBuyer(sku)
  {
